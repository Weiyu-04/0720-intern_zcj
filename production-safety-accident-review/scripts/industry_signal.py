...
#: 工序/商贸/服务泛化词黑名单。**入选判据是语义的**：一个词若描述"在做什么活动/功能"（跨
#: 行业通用的作业动作、商贸环节、服务功能），而**不指向任何具体行业**，就入选——正是
#: references/05 说的"活动≠主营"陷阱，与 GENERIC_EQUIPMENT 同理，不应算作"行业信号"。
#: （按此判据入选，而非按"是否命中某条 selftest"——如"工程/租赁/咨询"未出现在任何 selftest
#: 短语里，仍因其泛化语义入选；且"租赁/咨询"与 classify_org._CONTEXT_NEG 的降级词保持一致口径。）
#: 它们多是 _vocab() 切碎国标长名（"农副食品加工业"→"加工"）时漏进来的**碎片**，各行业的
#: **完整名仍在词表里**（只删裸碎片、不删任何具体行业名），故剔除只消掉子串误命中、不丢真实
#: 行业词。命中 → 按"信号真空"路由、强制走 verify（失效偏严，符合闸门"拿不准就更严"取向）。
#: 集合里少数词在当前国标版本里并非独立切词（拉黑=空转、无害），保留是因其语义确属泛化词、
#: 且换数据版本后可能出现——按**语义**而非"当前是否生效"定成员。
#: 硬判据（不变式，见 _selftest）：**只收非国标完整名的泛化碎片，绝不收任何国标完整行业名**。
#: 故如"安全"(碎自"安全、消防用金属制品制造"、无"安全业")、"工业/养护/管护"等纯碎片才入选；
#: 而"教育"虽在本域几乎每案以"安全教育"出现、极像噪声，但它**是**国标完整行业名(教育业)，删它
#: 会孤儿化真实行业——"安全教育"这类误命中属"活动≠主营"的**语义层**问题，交 verify+模型判，
#: 机械闸门不抹真行业。"安全"尤为关键：本域正是生产**安全**事故，几乎每份警情都含"安全"，留在
#: 词表里会近乎架空真空闸门。
GENERIC_PROCESS = frozenset((
    "加工", "制造", "生产", "服务", "经营", "销售", "批发", "零售", "仓储",
    "供应", "分配", "综合", "修理", "维修", "维护", "装修", "管理", "商务",
    "供销", "物资", "经销", "代理", "工程", "租赁", "咨询",
    "安全", "工业", "养护", "管护", "环保",
))
...
def _vocab() -> set[str]:
    ...
    剔除 GENERIC_PROCESS 里的工序/商贸泛化碎片——它们描述"活动"不描述"行业"，
    留着会让 scan 对"加工车间/制造车间/服务区"这类通用措辞误报"有行业信号"，
    从而软化本闸门主打的反臆造保护。剔除只消掉裸碎片，各行业完整名仍在词表内。
    """
    global _VOCAB
    if _VOCAB is None:
        ...
        for rec in codes.values():
            for tok in re.split(r"[、，（）()和及与\s]+", rec.get("name", "")):
                tok = tok.strip()
                if len(tok) >= 2 and tok not in GENERIC_PROCESS:
                    v.add(tok)
        _VOCAB = v
    return _VOCAB
...
def scan(text: str, name: str = "") -> dict:
    ...
    if vacuum:
        ...
        action = ("按 references/05 判据①②正常判定。注意：②公司名**绝不单独定案**，须回到①事故经过印证；"
                  "①②矛盾时以①为准。**『单位主营』一栏仍须跑 `verify` 校验其逐字出处**——本 scan 的词表可能"
                  "把'加工/制造/服务'等通用词误当行业信号，故'有信号'不等于'主营已有出处'：verify 说无出处，"
                  "则该栏只能填'未掌握'、按 references/05 §29 列候选，禁止给唯一四位小类与确定 WSA 编码。")
...
def _selftest() -> None:
    ...
    # --- scan：工序/商贸泛化词（加工/制造/服务/批发/综合仓储…）只说"在做什么活动"、不说"属什么
    #     行业"（活动≠主营），与通用设备词同理，不算行业信号 → 判真空、强制走 verify。
    #     多组不同泛化词，锁定的是"泛化词整类不算信号"这个通用性质，而非"某个词被拉黑"。
    for phrase in ("加工车间", "制造工段", "服务区后场", "批发库房", "综合仓储区",
                   "工程部现场", "设备租赁堆场", "咨询室"):
        rg = scan(f"某厂{phrase}内，一名工人作业时被砸致死。", "某某发展有限公司")
        assert rg["信号真空"] is True, (phrase, rg)
    # --- scan：本域高频噪声词（安全/工业/养护…）不得算行业信号，否则近乎架空真空闸门。
    #     "安全"尤关键：生产安全事故几乎每案必含"安全"。（"教育"是真实行业名，故意保留、不在此。）
    assert scan("某公司院内，一人被高处坠落物砸中致死，现场安全措施缺失。", "某某发展有限公司")["信号真空"] is True
    assert scan("某公司因安全生产责任落实不到位，一人高处坠落致死。", "某某发展有限公司")["信号真空"] is True
    assert scan("某公司养护班组作业时，一名工人从高处坠落致死。", "某某发展有限公司")["信号真空"] is True
    assert scan("某工业园区院内，一名工人检修时被高处坠物砸中致死。", "某某发展有限公司")["信号真空"] is True
    # "环保"已在 NAME_NOISE(名称层当噪声)，text 层也须一致当噪声，别一模块噪声、另一模块正信号
    assert scan("某公司环保设施检修时，一名工人中毒致死。", "某某发展有限公司")["信号真空"] is True
    assert "环保" in NAME_NOISE and "环保" in GENERIC_PROCESS  # 跨模块口径一致
    # --- scan：具体行业词（非泛化）仍照常算信号，别把真信号一起误杀
    rk = scan("某公司冶炼车间，一名炉前工被钢水烫伤致死。", "某某发展有限公司")
    assert rk["信号真空"] is False and "冶炼" in rk["经过_行业指示词"], rk
    # --- 不变式：GENERIC_PROCESS 只删"碎片"，绝不含任何国标完整行业名（防误伤真行业/孤儿化）。
    #     若今后有人把"教育"等真实行业名误加进来，此断言会红。
    with open(_DATA, encoding="utf-8") as _f:
        _all_names = {r.get("name", "") for r in json.load(_f)["codes"].values()}
    assert not (GENERIC_PROCESS & _all_names), ("误删国标完整行业名:", GENERIC_PROCESS & _all_names)

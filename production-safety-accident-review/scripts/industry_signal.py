...
#: 工序/商贸泛化词黑名单：这些是"在做什么活动/功能"的通用词（跨行业通用、描述作业动作或
#: 商贸环节），**本身不指向任何具体行业**（正是 references/05 说的"活动≠主营"陷阱），与
#: GENERIC_EQUIPMENT 同理，不应算作"行业信号"。它们是被 _vocab() 切碎国标长名（如"农副食品
#: 加工业"→"加工"）时漏进来的**碎片**，而各行业的**完整名仍在词表里**（只删裸碎片、不删任何
#: 具体行业名），故剔除它们只消掉子串误命中（"加工车间"误判有信号），不丢任何真实行业词。
#: 命中这些词 → 按"信号真空"路由，强制走 verify（失效方向偏严，符合闸门"拿不准就更严"的取向）。

...
**第1步 · 取干净正文并定位章节**：先按上传格式选对提取方式（`extract_clean.py` 只吃**文本型 PDF**，靠字号剥脚注/页码；Word 与扫描件它抽不了）：
- **文本型 PDF**（最常见）：**运行 `python <本技能目录>/scripts/extract_clean.py <上传PDF路径> /mnt/user-data/workspace/clean_report.txt`**，得到剥离页码/脚注、劈开句子已拼回的干净正文（脚注混入会严重干扰解析，务必先做）。
- **Word（.docx）/ 扫描件或图片型 PDF / 其它格式**：`extract_clean.py` 处理不了，改用平台工具 **`parse_document(<上传文件路径>)`** 转成文本（Word 直接转、扫描件走 OCR），把结果落到 `/mnt/user-data/workspace/clean_report.txt` 再往下走。此路无字号信息、不会自动剥脚注，**你需人工留意并跳过页眉/页脚/脚注行**。
- 已是纯文本：直接用。

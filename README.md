...
### 2.1 本轮（0721 交接复核会话）——研判助手启发式层加固

接手复核：通读两个技能全部脚本/references/模板，在干净容器**实测确定性内核**（后评估端到端产 4 文件、md5 恒定；研判 `grade` 三维全边界对齐 493 号、`verify` 反臆造闸门、`encode`/`geo` 均正确；数据 `gbt4754` 1381 小类 / `geo` 省地县自洽；「答案不泄漏」铁律通过）。**未发现阻塞问题**，修复了 4 处非阻塞小问题（全在模糊启发式**辅助层**，失效方向偏保守，确定性内核未动）：

- **`industry_signal` 泛化词弱化闸门 [中]**：`_vocab()` 把国标长名切碎时漏进了"加工/制造/服务/工程/租赁/咨询/**安全**/工业/养护…"等跨行业泛化碎词，使 `scan` 对"加工车间/工程部/现场安全措施"等通用措辞误报"有行业信号"、落入 verify 可选的软分支。**"安全"尤为致命**——本域正是生产安全事故、几乎每案必含"安全"，留着近乎架空真空闸门。修法：加 `GENERIC_PROCESS` 黑名单（**硬判据=只收"非国标完整名的泛化碎片"，绝不收任何完整行业名**；故"安全/工业/养护"等纯碎片入选，而"教育"虽常以"安全教育"误命中、却因**是**真实行业名教育业而**保留**——该类误命中属"活动≠主营"的语义层问题，交 verify+模型判，机械闸门不抹真行业；此不变式已加 selftest 锁死）；并做**跨模块口径对齐**——"租赁/咨询"不再一边当正信号一边当降级词（`classify_org._CONTEXT_NEG`），"环保"不再一边当噪声（`NAME_NOISE`）一边当正信号；再把"有信号"分支的 verify 从"可选自查"收紧为"主营必须验逐字出处"（与 SKILL「verify 每案必调」一致，纵深防御）。
- **`classify_org` "城"误伤地名 [中/低]**：裸字"城"本为抓"建材城/食品城"卖场后缀，却误命中盐城/晋城/运城等地名、错误降置信。改成"行业词+城"相邻判定，既保原意又不伤地名（全库扫描确认无真实地名含"〈行业词〉城"子串）。
- **`geo` 通用地域词县名 [低]**：由纯方位/通用地域词构成的区名（老城区/东区/西区/城东区/城西区/城北区/新市区 等，库中唯一者会被静默确指）+ 无"与归一一致"的省市锚点时，归一加"注意"提示（**只提示、不改归一**）。抑制条件按**锚点一致性**判（文本给了与归一冲突的省/市——如"广东省老城区"归一到河南洛阳——照样提示，不被误抑制）。
- **`grade` 健壮性 [低]**：校验加 `bool` 拦截（`True` 曾会被当 1 人死亡）、非数值损失给友好报错。
- 四处均补 selftest 锁定（含"只删碎片不删完整行业名"的不变式测试）；全量 selftest（研判 5 脚本）+ 后评估流水线回归**通过**；并经**多轮对抗性复核**（正确性/铁律视角 + 过拟合/回归视角迭代两次），逐轮查出并回填：A-1（工程/租赁 漏网+跨模块矛盾）、C-2（东区/城北区等唯一泛区名漏收）、C-4（冲突锚点误抑制提示）、以及复验轮揪出的**"安全"碎片近乎架空闸门**（连同养护/工业一并拉黑，"教育"因是真实行业名而保留）。

> 三项待办仍卡在带教/平台侧（本仓库够不到）：待办#1 新案例泛化验证的**模型语义部分**需真实报告+平台跑（确定性解析层已用多排版压力测试验证泛化）；待办#2 表3 真实界面端到端需 MainAgent 平台；待办#3 上线流程需公司确认。另：研判技能 `allowed-tools` 含 `write_file` 而该技能只读、不产文件，属可选的最小权限收窄——因作者曾刻意收窄过工具面、其运行期效果本仓库又无法验证，未擅改，留带教定夺。
...
7. **（未做）OCR 路用 MinerU 版面输出替代字号剥脚注**：识别字号的**目的**是把正文和脚注/页眉/页码分开（下游按句号拆行，脚注混入会被当成一条整改要求）。图片/扫描件没有字号，但其**等价信号是"视觉版面"**——MinerU 本身是版面感知解析器，按位置/相对大小识别脚注/页眉，`parse_document` 还写了 `<名>_middle.json` 记录块级版面。故比 `md_to_clean` 的文本模式更稳的做法：**读 MinerU 的版面输出、按块类型剥脚注/页眉**（需先确认 MinerU 的块类型 schema）。当前先用 md_to_clean 文本模式 + 模型兜底校验近似。

> **交给新窗口的进一步优化（非阻塞）**：待办 6（乱码检测 → OCR 回退）、待办 7（用 MinerU 版面输出替代字号剥脚注）——这两项是让 OCR 路更接近 PDF 路的增强，留给新窗口。**边界：OCR 认错字（人名/数字/生僻字）属 OCR 的固有限制，本技能不负责修正**，由用户核对成品时自行处理，新窗口不必在此耗力（模型第2/3步只做力所能及的核对，不承诺修全）。

...
- **`industry_signal` 泛化词弱化闸门 [中]**：`_vocab()` 把国标长名切碎时漏进了"加工/制造/服务/批发/仓储…"等跨行业工序/商贸泛化碎词，使 `scan` 对"加工车间"等通用措辞误报"有行业信号"、落入 verify 可选的软分支。修法：加 `GENERIC_PROCESS` 黑名单剔除这些碎词（**只删裸碎片、各行业完整名仍在**，实测仅删 11 词、零具体行业名被误删）；并把"有信号"分支的 verify 从"可选自查"收紧为"主营必须验逐字出处"（与 SKILL「verify 每案必调」一致，纵深防御）。
- **`classify_org` "城"误伤地名 [中/低]**：裸字"城"本为抓"建材城/食品城"卖场后缀，却误命中盐城/晋城/运城等地名、错误降置信。改成"行业词+城"相邻判定，既保原意又不伤地名。
- **`geo` 通用地域词县名 [低]**：`老城区/城区/郊区` 这类既是行政区名又是泛地域说法的县名，无省市锚点时归一加"注意"提示（**只提示、不改归一**）。
- **`grade` 健壮性 [低]**：校验加 `bool` 拦截（`True` 曾会被当 1 人死亡）、非数值损失给友好报错。
- 四处均补了 selftest 锁定；全量 selftest（研判 5 脚本）+ 后评估流水线回归**通过**。

...
- **`industry_signal` 泛化词弱化闸门 [中]**：`_vocab()` 把国标长名切碎时漏进了"加工/制造/服务/批发/仓储/工程/租赁/咨询…"等跨行业工序/商贸/服务泛化碎词，使 `scan` 对"加工车间/工程部/设备租赁堆场"等通用措辞误报"有行业信号"、落入 verify 可选的软分支。修法：加 `GENERIC_PROCESS` 黑名单（**入选按语义判据=跨行业功能词**，剔除 14 个碎片，**只删裸碎片、各行业完整名仍在、零具体行业名被误删、零国标码孤儿化**，且与 `classify_org._CONTEXT_NEG` 的降级词对齐口径——如"租赁/咨询"不再一个模块当正信号、另一模块当降级词）；并把"有信号"分支的 verify 从"可选自查"收紧为"主营必须验逐字出处"（与 SKILL「verify 每案必调」一致，纵深防御）。
...
- 四处均补 selftest 锁定；全量 selftest（研判 5 脚本）+ 后评估流水线回归**通过**；并经**两轮对抗性复核**（正确性/铁律视角 + 过拟合/回归视角），第二轮查出的 A-1（工程/租赁 漏网+跨模块矛盾）、C-2（东区/城北区等唯一泛区名漏收）、C-4（冲突锚点误抑制提示）已全部回填。
